...
//		this.minecraft.keyboardHandler.setSendRepeatsToGui(true);
		...
		url = addRenderableWidget(new EditBox(font, guiLeft + 7, guiTop + 19, 142, 18, Component.empty()));
		url.setTooltip(Tooltip.create(OnlineDisplays.gui("url")));
		...
		
		{
			...
			tx = addRenderableWidget(new EditBox(font,
					guiLeft + xBase, guiTop + yBase, 40, 18, Component.empty()
			));
			tx.setTooltip(Tooltip.create(OnlineDisplays.gui("translate_x")));
			tx.setMaxLength(10);
			tx.setValue(pv);
			tx.moveCursorToStart();
			
			pv = ty != null ? ty.getValue() : Float.toString(display.matrix.translateY);
			ty = addRenderableWidget(new EditBox(font,
					guiLeft + xBase + 60, guiTop + yBase, 40, 18, Component.empty()
			));
			ty.setTooltip(Tooltip.create(OnlineDisplays.gui("translate_y")));
			ty.setMaxLength(10);
			ty.setValue(pv);
			ty.moveCursorToStart();
			
			pv = tz != null ? tz.getValue() : Float.toString(display.matrix.translateZ);
			tz = addRenderableWidget(new EditBox(font,
					guiLeft + xBase + 120, guiTop + yBase, 40, 18, Component.empty()
			));
			tz.setTooltip(Tooltip.create(OnlineDisplays.gui("translate_z")));
			tz.setMaxLength(10);
			tz.setValue(pv);
		...
		
		{
			...
			rx = addRenderableWidget(new EditBox(font,
					guiLeft + xBase, guiTop + yBase, 40, 18, Component.empty()
			));
			rx.setTooltip(Tooltip.create(OnlineDisplays.gui("rotate_x")));
			rx.setMaxLength(10);
			rx.setValue(pv);
			rx.moveCursorToStart();
			
			pv = ry != null ? ry.getValue() : Float.toString(display.matrix.rotateY);
			ry = addRenderableWidget(new EditBox(font,
					guiLeft + xBase + 60, guiTop + yBase, 40, 18, Component.empty()
			));
			ry.setTooltip(Tooltip.create(OnlineDisplays.gui("rotate_y")));
			ry.setMaxLength(10);
			ry.setValue(pv);
			ry.moveCursorToStart();
			
			pv = rz != null ? rz.getValue() : Float.toString(display.matrix.rotateZ);
			rz = addRenderableWidget(new EditBox(font,
					guiLeft + xBase + 120, guiTop + yBase, 40, 18, Component.empty()
			));
			rz.setTooltip(Tooltip.create(OnlineDisplays.gui("rotate_z")));
			rz.setMaxLength(10);
			rz.setValue(pv);
		...
		
		{
			...
			sx = addRenderableWidget(new EditBox(font,
					guiLeft + xBase, guiTop + yBase, 40, 18, Component.empty()
			));
			sx.setTooltip(Tooltip.create(OnlineDisplays.gui("scale_x")));
			sx.setMaxLength(10);
			sx.setValue(pv);
			sx.moveCursorToStart();
			
			pv = sy != null ? sy.getValue() : Float.toString(display.matrix.scaleY);
			sy = addRenderableWidget(new EditBox(font,
					guiLeft + xBase + 60, guiTop + yBase, 40, 18, Component.empty()
			));
			sy.setTooltip(Tooltip.create(OnlineDisplays.gui("scale_y")));
			sy.setMaxLength(10);
			sy.setValue(pv);
			sy.moveCursorToStart();
		}
		
		addRenderableWidget(Button.builder(OnlineDisplays.gui("apply"), btn -> applyChanges())
				.bounds(guiLeft + xSize - 80 - 6, guiTop + ySize - 26, 80, 20)
				.tooltip(Tooltip.create(OnlineDisplays.gui("apply")))
				.build());
		
		addRenderableWidget(Button.builder(OnlineDisplays.gui("aspect"), btn -> applyAspectRatio())
				.bounds(guiLeft + 6, guiTop + ySize - 26, 80, 20)
				.tooltip(Tooltip.create(OnlineDisplays.gui("aspect")))
				.build());
		
		addRenderableWidget(new ImageButton(guiLeft + 7 + 143, guiTop + 18, 20, 20,
				0, 0,
				20,
		...
		)).setTooltip(Tooltip.create(OnlineDisplays.gui("local_file")));
		
		emissiveToggle = (ImageButtonAccessor) addRenderableWidget(new ImageButton(guiLeft + 7, guiTop + 117, 20, 20,
				0, 0,
				20,
		...
		((ImageButton) emissiveToggle).setTooltip(Tooltip.create(OnlineDisplays.gui("emissive")));
...
//		this.minecraft.keyboardHandler.setSendRepeatsToGui(false);
	...
	@Override
	public void render(GuiGraphics mat, int mouseX, int mouseY, float partial)
	{
		renderBackground(mat);
		
		mat.blit(texture, guiLeft, guiTop, 0, 0, xSize, ySize);
		mat.drawString(font, getTitle(), guiLeft + 8, guiTop + 6, 0x3F3F3F, false);
