...
public class D0
{
	public static GIFFrame[] decode(IFileContainer file)
	{
		if(file.exists())
		{
			try(InputStream in = file.openInput())
			{
				...
				d.read(in);
				GIFFrame[] frames = new GIFFrame[d.getFrameCount()];
				for(int i = 0; i < frames.length; ++i)
					frames[i] = new GIFFrame(d.getFrame(i), d.getDelay(i));
				return frames;
	...
	public static class GifDecoder
	{
		...
		public BufferedImage getFrame(int n)
		{
			...
			if(n >= 0 && n < this.frameCount)
			{
				im = this.frames.get((int) n).image;
			}
			return im;
		}
		...
		protected void readHeader()
		{
			...
			for(int i = 0; i < 6; ++i)
			{
				id = String.valueOf(id) + (char) this.read();
			}
